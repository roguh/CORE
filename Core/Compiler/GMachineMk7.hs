...
module Core.Compiler.GMachineMk7
...
evalMk7 state = do
    restStates <- if isFinal state then return []
                  else liftM doAdmin (step state) >>= evalMk7
...
step :: GMStateMk7 -> ThrowsError GMStateMk7
...
compileMk7 :: [Supercombo Text] -> ThrowsError GMStateMk7
compileMk7 program = do
    (heap, globals) <- do
